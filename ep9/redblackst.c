...
void
freeNode(struct node *x);

void
cleanUpNodes(RedBlackST st, struct node *x, void *lo, void *hi);
/*---------------------------------------------------------------*/
static Bool
isBST(RedBlackST st);
...
freeST(RedBlackST st)
{
    ...
    */
    ...
    minNode = min(st);
    maxNode = max(st);
    cleanUpNodes(st, st->root, minNode, maxNode);
...
cleanUpNodes(RedBlackST st, struct node *x, void *lo, void *hi){
    ...
    cmplo = st->compar(lo, x->key); 
    cmphi = st->compar(hi, x->key);
    if (cmplo < 0) cleanUpNodes(st, x->left, lo, hi); 

    if (cmphi > 0) cleanUpNodes(st, x->right, lo, hi); 
    if (cmplo <= 0 && cmphi >= 0){
        freeNode(x);
    } 
}
